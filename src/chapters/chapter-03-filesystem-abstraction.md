...
Linux の「すべてはファイル」という思想は、コンピュータの複雑な要素を「ファイル」という単純な概念に抽象化する、革命的なアイデアである。
...
Linux のファイルシステムは、以下のような階層構造で組織されています：

![Linux ファイルシステム階層]({{ '/assets/images/diagrams/chapter-03/linux-filesystem-hierarchy.svg' | relative_url }})
...
#### /dev/null - ブラックホール
すべてのデータを受け取って破棄する特殊なデバイス。出力を捨てたいときに使う。
...
#### /dev/zero - 無限のゼロ
読み込むとずっと「0」のバイト列が返ってくるデバイス。ファイルの初期化や容量確保に利用。
...
環境ノイズなどを元にしたランダムなデータを生成。
/dev/random は高品質な乱数を提供するが、エントロピー不足でブロックすることがある。
/dev/urandom は擬似乱数を高速に返し、一般的な用途で十分な安全性がある。
...
パイプ（|）は、コマンドの標準出力を直接次のコマンドの標準入力に接続する。カーネルが管理するメモリ上のバッファを介してプロセス間でデータを受け渡す：
...
3. **Kubernetes**：すべての設定を YAML ファイルで管理
...
RAID 0（ストライピング）：
データを複数のディスクに「ストライプ（帯）」状に分割し、同時に書き込み・読み込みを行い高速化を図る方式。冗長性はなく、1台でも故障すると全データが失われる。

RAID 1（ミラーリング）：
同じデータを2台以上のディスクに同時保存し、耐障害性を高める方式。容量効率は半分だが、1台故障しても運用可能。

RAID 5（分散パリティ）：
データとパリティ（誤り訂正情報）を複数ディスクに分散して保存。高速性と耐障害性のバランスが良く、1台の故障まで耐えられる。

RAID 6（ダブルパリティ）：
RAID5の仕組みに加えて、パリティを2つ分保存する方式。2台まで同時故障に耐えられるが、書き込み速度は RAID5より遅い。

RAID 10（1+0、ミラー＋ストライピング）：
複数の RAID1（ミラー）を組み合わせて、その上で RAID0（ストライピング）を行う方式。高速性と耐障害性の両立が可能。ただし最低4台必要で容量効率は50%。

![RAID 構成の特徴比較]({{ '/assets/images/diagrams/chapter-03/raid-comparison.svg' | relative_url }})
...
| RAID 10     | 4             | あり   | 高速          | 50%      | 高性能 DB |

### 次章への準備

次章では、これらの「ファイル」を実際に操作する方法を学ぶ。シェルとコマンドラインこそが、Linux システムと対話する最も強力な方法である。

なぜ GUI ではなく CUI が主流なのか。その答えは、自動化、再現性、そして効率性にある。小さなコマンドを組み合わせて、複雑なタスクを解決する。その技術を、次章で身につけていこう。
...
1. Linux の「すべてはファイル」という思想では、デバイスは（　　　）ディレクトリに、プロセス情報は（　　　）ディレクトリに、カーネルパラメータは（　　　）ディレクトリにファイルとして存在する。
...
1. なぜ Linux ではハードウェアデバイスをファイルとして扱うのか、その利点を3つ挙げて説明してください。
2. /procファイルシステムと/sysファイルシステムの違いと、それぞれの用途を説明してください。
3. ブロックデバイスとキャラクタデバイスの違いを、具体例を挙げて説明してください。

### 問題3：実践的思考
以下のタスクを Linux のファイルシステムを使って実現する方法を説明してください。
...
### 問題6：発展的課題
1. Docker やコンテナ技術が、Linux の「すべてはファイル」という思想をどのように活用しているか考察してください。特に、cgroupsとnamespacesの実装について言及してください。
