...
Linuxは、安定性や性能、セキュリティなどの要件を高いレベルで満たすことが多く、そのためサーバーOSとして世界中の多くの企業で広く採用されています。
...
## 1.4 なぜ企業でLinuxが広く使われるのか

### 歴史的背景：プロプライエタリからオープンソースへ

1990年代、企業のサーバーは主に商用UNIX（Solaris、AIX、HP-UXなど）やWindows Serverで運用されていました。これらのプロプライエタリソフトウェア（特定企業が権利を持ち、利用や改変に制限がある商用ソフトウェア）は、高価でしたが、ベンダーのサポートがあるため採用されていました。
...
一般的に、nginxやApacheはLinux上で広く使われているWebサーバーソフトウェアである。
