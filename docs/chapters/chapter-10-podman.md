...
2013年、Dockerの登場によりコンテナ技術は爆発的に普及しました。しかし、エンタープライズ環境での利用が進むにつれ、いくつかの課題が明らかになりました。
...
> 💡 この章の読み方  
> はじめてコンテナ技術に触れる場合は、まずは「Docker とほぼ同じ操作感で使える」「一般ユーザー権限でコンテナを動かせる（Rootless）」という2点だけ押さえて読み進めてください。SELinux 連携などの細かなセキュリティ機能については、2回目以降の読み返しで徐々に理解していく前提でも構いません。
