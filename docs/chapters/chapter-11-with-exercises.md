...
- 必須コマンド: 本章の JSON 出力の確認には `jq` が必要
- 演習（第11章以降）: 必要に応じて、コンテナレジストリなどの認証情報（アカウント/アクセストークン等）が必要になる場合があります。各章で示す前提に従って準備してください。

`jq` を使う前に、実行ファイルの存在とバージョンを確認します。
...
文書化した検証環境の Ubuntu で未導入の場合は、Ubuntu のパッケージリポジトリから導入します。他のディストリビューションでは、そのディストリビューションの公式パッケージリポジトリを使用してください。
...
```bash
# イメージのレイヤーを表示
$ podman image inspect nginx:1.27-alpine | jq '.[0].RootFS.Layers'
[
...
注記: ここでの `nginx:1.27-alpine` は layer 観察用の固定タグ例です。CI / deploy へ持ち込むイメージ参照は、検証済みの固定タグまたは digest に置き換えてください。
...
[1/4] FROM node:22-alpine                    # 固定タグのベースイメージを取得
...
注記: この scan 例は Docker socket が利用できる runner を前提にしています。Podman / rootless を前提にする場合は、registry 上の `$IMAGE_NAME` を直接スキャンできるモードや daemon socket を要求しない scanner を選び、build/test/scan/deploy で前提ランタイムが混線しないようにしてください。
...
# イメージのビルド
echo "Building image..."
podman build -f Dockerfile.analysis -t layer-test:analysis .

# レイヤー情報の表示
echo -e "\nLayer information:"
podman history layer-test:analysis

# 各レイヤーの内容を確認
echo -e "\nExtracting layers..."
podman save layer-test:analysis -o layer-test.tar
mkdir -p extracted
...
podman rmi layer-test:analysis
EOF
...
# registry から取得できることを確認
podman rmi localhost:5000/test-image:v1
podman pull localhost:5000/test-image:v1
podman image inspect --format '{% raw %}{{.RepoDigests}}{% endraw %}' localhost:5000/test-image:v1

# 注記: `:latest` は複数タグの動きを確認するための例です。deploy や promotion の起点は `v1` のような固定タグまたは digest を使ってください。
# 注記: この手順は localhost 限定の lab です。外部公開が必要な場合は TLS / 認証付き構成へ進み、`-p 127.0.0.1:5000:5000` を無条件で広げないでください。

# クリーンアップオプション
echo -e "\nTo clean up:"
echo "  podman stop registry && podman rm registry"
echo "  podman volume rm registry-data"
echo "  podman rmi localhost:5000/test-image:v1 localhost:5000/test-image:latest"
echo "  rm -f Dockerfile"
EOF
...
注記: ここでも固定タグを使っていますが、継続運用のスキャン対象は承認済みの固定タグまたは digest に合わせてください。`latest` のような可変タグを scan 対象へ残すと、同じレポートを後から再現しにくくなります。
...
4. **CI/CD との統合**：自動化されたビルド・テスト・デプロイ
...
これまで Linux の基礎からコンテナまでを学んできました。次章からは、これらの技術が「クラウド」という新しいインフラでどのように活用されるかを見ていきます。

AWS という巨大なデータセンターが、どのようにコンピューティングリソースを抽象化し、提供しているのか。そして、私たちがどのようにそれを活用できるのか。クラウド時代のインフラ技術を探求していきましょう。
...
2. Dockerfile の`COPY`命令と`ADD`命令の違いは、`ADD`命令が（　　　）や（　　　）の機能を持つことです。
...
1. **レイヤーキャッシュの仕組みについて説明してください。なぜ Dockerfile の命令の順序が重要なのですか？**
...
- ビルド時間の短縮
- セキュリティの向上
- version pinning と trusted base image の明示

### 問題4：レイヤー分析

次の Dockerfile からビルドされるイメージについて：
...
以下の要件を満たすGitLab CI/CD パイプラインを設計してください：
...
以下の Dockerfile のセキュリティ上の問題点を指摘し、改善案を提示してください：
...
2. **OCI（Open Container Initiative）仕様について調査し、Docker イメージフォーマットとの関係を説明してください。**
